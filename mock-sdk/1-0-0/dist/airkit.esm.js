...
class FlattenedSign {
  ...
  async sign(key, options) {
    ...
    const extensions = validateCrit(JWSInvalid, /* @__PURE__ */ new Map([["b64", true]]), options == null ? void 0 : options.crit, this._protectedHeader, joseHeader);
    let b64 = true;
    if (extensions.has("b64")) {
...
class SignJWT extends ProduceJWT {
  ...
  async sign(key, options) {
    ...
    if (Array.isArray((_a = this._protectedHeader) == null ? void 0 : _a.crit) && this._protectedHeader.crit.includes("b64") && this._protectedHeader.b64 === false) {
      throw new JWTInvalid("JWTs MUST NOT use unencoded payload");
    }
...
class AirService {
  ...
  async login({ authToken } = {}) {
    if (authToken) {
      ...
      return new Promise((resolve) => {
        import("./loginDialog-CyIwgU9X.mjs").then((dialog) => {
  ...
  async generateToken(userData) {
    const secretKey = await importJWK(SECRET_KEY_JWK, "HS256");
    return await new SignJWT(userData).setProtectedHeader({ alg: "HS256" }).setExpirationTime("1h").sign(secretKey);
  }
  /**
...
const airConnector = ({ partnerId }) => createConnector((config) => {
